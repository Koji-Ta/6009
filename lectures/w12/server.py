...
def send_response(sock, message, type_, code="200 OK"):
    ...
    # now send a response
    sock.sendall(b'HTTP/1.1 ' + code.encode() + b'\r\n')
    sock.sendall(b'Content-type: ' + type_.encode() + b'\r\n')
...
def url_decode(x):
    """
    Decode an input string that has been URL encoded.
    ...
    while ix < len(x):
        if x[ix] == '%':
            charcode = x[ix+1:ix+3]
            out += chr(int(charcode, base=16))
            ix += 3
        else:
            out += x[ix]
            ix += 1
...
while True:
    ...
    req = connected_socket.recv(4096).decode()
    ...
    if req_tokens[0] == 'GET':
        ...
        # if the location contains a query string, we should parse that out.
        # this piece of code will make a dictionary containing the given
        # parameters, which we can then pass to the relevant Python function.
        if '?' in location:
            location, qstring = location.split('?', 1)
            params = dict(i.split('=') for i in qstring.split('&'))

            # certain characters in query string parameters are 'escaped'.
            # here we call out to a helper function to decode those characters.
            # see https://en.wikipedia.org/wiki/Query_string#URL_encoding
            params = {url_decode(k): url_decode(v) for k,v in params.items()}
        else:
            # if no parameters were specified, we still want a dictionary we
            # can pass into the relevant function.  default to an empty
            # dictionary.
            params = {}
