...
#
def select_candidates(num_candidates, num_talents, candidate_to_talents):
    global call_count
    call_count = 0
...
def brute_select_candidates(num_candidates, num_talents, candidate_to_talents):
    # use sets for fast intersections and differences
    c_to_t = [set(candidate_to_talents[c]) for c in range(num_candidates)]
...
def ordered_select_candidates(num_candidates, num_talents, candidate_to_talents):
    # use sets for fast intersections and differences
    c_to_t = [set(candidate_to_talents[c]) for c in range(num_candidates)]
...
if __name__ == '__main__':
    ...
    candidate_to_talents = [[0, 3, 6], [3, 4, 5], [6, 7, 8], [9, 10, 11], 
                            ...
                            [14], [9, 12, 15]]
    select_candidates(num_candidates, num_talents, candidate_to_talents)
    end = time.time()
